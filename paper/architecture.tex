...
and a trigger. The payload, and some variants of the trigger, must be  installed
on the victim's computer(e.g. via trojan, remote exploit, or phishing). A running
payload performs innocuous measurements while waiting for the trigger to
become present. 

If the trigger is a local program interacting with the payload
via the indirect branch predictor both programs must run on the same physical CPU. We 
note that this constraint is not a significant burden as most OS schedulers
will use round robin algorithms to assign tasks to processors---eventually 
the trigger and payload will execute co-resident on a single core.
...
If the trigger is an input the program parses innocuous entries waiting 
for a series of crafted entries to train the direct branch predictor 
before a specially crafted trigger packet. The payload then forces the CPU
to speculate by stalling a bounds check. A local array is then speculatively 
overflowed redirecting program execution speculatively 
to an address within the payload program specified by the specially 
crafted trigger packet provided by the attacker.
