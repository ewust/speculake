...
existing anti-virus
systems~\cite{jana2012abusing,oberheide2009polypack,ropinjector,swinnen2014one}.


We assume the adversary is able to install binaries on the target machine
...
adaptive fuzzers~\cite{afl} to generate inputs that explore other execution
...
channels invisible to the analyst. As with typical malware, the analyst may
attempt to reverse engineer the trigger to revela the malware's behavior, but we
...
program will speculate a second time and continue speculative execution at C.
...
to thoroughly protect speculative decryption keys from reverse
engineering.
