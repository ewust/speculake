...
secret data from a process or operating system. Recent follow-up work has
revealed other Spectre ``variants'',  including speculative buffer
overflows, speculative store bypass, and using alternative side channels besides
the cache~\cite{spectre1-1,spectre3}. In addition, several attacks have
leveraged Spectre to attack Intel's SGX~\cite{sgx-spectre,foreshadow}, and
perform remote leakage attacks~\cite{netspectre}.
