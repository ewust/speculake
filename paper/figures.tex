...
\begin{figure*}[t]
    ...
    instructions we can execute speculatively we placed a signal gadget after an
    increasing number of instructions and measured the fraction of times the
    loaded value was subsequently in cache. We identify two limitations on the
    number of instructions accomplishable speculatively: cache latency, and the
    CPU's reorder buffer~(ROB) size. We find that the hit fraction for
    ...
    Figure~\ref{fig:cache-miss} (approx. 200 cycles). Whether or not the
