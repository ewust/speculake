...
\begin{figure}[t]
    ...
    \caption{When using speculative buffer overflow the direct branch predictor
    must be trained on input data before it can mis-speculate a branch. 
    Over an independent number of training 
    rounds we measure the resulting hit rate fraction of trials where the 
    speculative gadget is able to perform the intended cache load. We find 
    that around 200 benign training branches are required before a branch 
    can be mis-speculated with discernable hit rate fraction. Note that a 
    1\% signal is generally significant enough to discern as background cache noise 
    is minimal in an ideal scenario.}
...
\begin{figure*}[t]
    ...
    to be completed speculatively, dependent on multiple factors. The ROB 
    capacity aligns directly with the upper bound of nop instructions 
    that can be completed speculatively, while other constraints bound more 
...
\begin{figure}[b]
    ...
    \caption{Trigger and payload processes must share CPU resources as they 
        must be performed on the same hyperthread or associated parity hyperthreads. 
        Processes on the parity hyperthreads (cool colors) denoted by (P) accomplish a 
        significantly lower number of instructions as compared with processes 
        on the same hyperthread (warm colors).}
...
\begin{figure}[t]
    ...
    \caption{The Cache Hit/Miss Latency Distribution of an Intel
        Xeon-1270 and AMD Epyc 7251. Measured by timing accesses to cached/uncached memory. 
