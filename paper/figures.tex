...
\begin{figure}[t]
    ...
    direction at least 400 times before it can start to be used in a speculative buffer overflow.}
...
\begin{figure*}[t]
    ...
    was subsequently in cache. We find that we can execute up to 220 instructions speculatively,
    which corresponds with the size of the CPU's reorder buffer (ROB). However, more complex instructions
...
\begin{figure}[b]
     \centering
        ...
        \caption{\textbf{Hyperthreading}\,---\, %
        The trigger program influences the payload program via the indirect branch predictor
        when running on the same logical CPU core. Hyperthreads also share an indirect branch
        predictor despite running on different logical cores. We measured the impact of
        same logical core (warm colors) vs paritiy hyperthreads (cool colors, denoted by (P))
        on the size of the speculative execution window. As hyperthreads run concurrently, they do not
        need to wait for an OS context switch between processes, but the two processes directly
        contend for ROB entries. As a result, we find that hyperthreads generally have a smaller
...
\begin{figure}[t]
    \centering
        ...
        Using the speculative primitive, 1KB of data
        ...
        performance factors, with an optimal throughput achieved at 8~bits.}
...
\begin{figure}[b]
    \centering
        ...
        Our SPASM emulator speculatively decrypts intructions, and emulates
