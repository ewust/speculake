...
\begin{figure*}[t]
    ...
    loaded value was subsequently in cache. We identify two limitation on the number of instructions
    accomplishable speculatively -- cache latency, and the CPU's reorder buffer~(ROB) size. We find that the
    ...
    Figure~\ref{fig:cache-miss} (approx. 200 cycles). Whether or not the instruction sequence reaches the 
    ...
    instruction limited by ROB (not including ~15 instructions for signal gadget), with a soft limit at 
    12 instructions enforced by the cache latency. We measure the ROB size by timing two cache 
    accesses with a variable number of instructions between them and note that this aligns with 
    the measured maximum \texttt{nop} instrustions.
    }
